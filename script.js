...
async function parseAndApplyEffect(effectCode) {
    ...
    const ifMatch = ifRegex.exec(effectCode); // 使用 exec 更好地处理

    if (ifMatch) {
        const conditionCode = ifMatch[1];
        let conditionMet = false;
        try {
            // 动态评估条件。这里需要注意安全隐患，但考虑到是私有脚本，可以接受
            // 将 student.has_trait('trait_name') 转换为 student.traits.includes('trait
