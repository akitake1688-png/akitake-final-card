// script.js - 紧急修复最终版 (AI 留学咨询室 - 深度定制版 / 策略模拟系统)
...
// 修复：添加对游戏内按钮的引用
const btnDrawCards = document.getElementById('btnDrawCards');
...
const studentCards = [
    ...
    },
    {
        ...
        name: "潜力优秀生",
        initialAttributes: { gpa: 3.8, jlpt: 90, toefl: 95, logic_skill: 75, cultural_adaptability: 60, psychological_resilience: 70, confidence: 70, anxiety: 20, academic_score: 80, narrative_coherence: 70 },
...
const strategyCards = [
    ...
    {
        id: "strategy_confidence_001", name: "微实绩积累法", type: "strategy", subtype: "psychological_intervention", cost: { energy: 3 },
        effect_code: "student.confidence += 15; student.anxiety -= 10; GAIN energy=1;",
        description: "将大目标分解为小目标，通过持续获得小成功来积累自信，减轻压力。", flavor_text: "点滴积累，自信倍增。"
    },
    {
        id: "strategy_social_001", name: "导师印象管理", type: "strategy", subtype: "relationship_building", cost: { energy: 4 },
        ...
        description: "指导学生如何通过邮件、交流等方式，给潜在导师留下积极专业的印象。", flavor_text: "细节决定成败，印象定未来。"
    },
    {
...
const challengeCards = [
    {
        ...
        description: "学生的研究计划书主题过于宏大模糊，缺乏具体可行的研究切入点。", solution_strategy_id: "strategy_logic_001"
    },
    {
        id: "challenge_culture_001", name: "面试读空气失败", type: "challenge", category: "cultural_conflict",
        trigger: "student.cultural_adaptability < 50 && gameState.gamePhase === 'crisis_phase' && gameState.turn > 4",
        penalty: { cultural_adaptability: -15, confidence: -10, anxiety: 15 },
        description: "在教授面试中未能理解非语言暗示，导致沟通障碍，教授印象降低。", solution_strategy_id: "strategy_culture_001"
    },
    {
        id: "challenge_psych_001", name: "申请期焦虑症", type: "challenge", category: "psychological_pressure",
        trigger: "student.anxiety > 60 || (gameState.gamePhase === 'crisis_phase' && gameState.turn > 3)",
        penalty: { psychological_resilience: -10, confidence: -10, logic_skill: -5 },
        description: "长期申请压力导致学生焦虑情绪高涨，影响学习效率和决策判断。", solution_strategy_id: "strategy_psych_001"
    },
    {
        id: "challenge_motivation_001", name: "双非背景自卑", type: "challenge", category: "psychological_pressure",
        ...
        description: "学生因国内双非背景产生自卑心理，影响文书撰写和面试表现。", solution_strategy_id: "strategy_narrative_001"
    },
    {
        id: "challenge_communication_001", name: "教授回复冷淡", type: "challenge", category: "cultural_conflict",
        trigger: "student.cultural_adaptability < 60 && gameState.turn > 5 && Math.random() < 0.3",
        penalty: { confidence: -10, anxiety: 5, application_score: -5 },
        description: "教授对套磁邮件的回复过于官方或冷淡，学生感到挫败和迷茫。", solution_strategy_id: "strategy_social_001"
    }
];
...
const goalCards = [
    ...
    },
    {
        ...
        description: "日本私立双雄之一的经济学研究科，对申请者的逻辑思维和研究计划有较高要求。",
...
let gameState = {
    ...
    turn: 0, maxTurns: 10,
    playerEnergy: 10, playerInsight: 0, playerCredit: 0,
    ...
    phaseDescriptions: {
        ...
        "crisis_phase": { name: "危机期 (文化冲击)", description: "学生面对文化差异和压力，易出现焦虑。", challenge_odds_modifier: 1.5, psychological_strategy_bonus: 1.2 },
...
function updateStudentDashboard() {
    ...
    updateDashItemColor(dashGPA.parentNode, student.gpa, 3.7, 3.0); updateDashItemColor(dashJLPT.parentNode, student.jlpt, 90, 70); updateDashItemColor(dashTOEFL.parentNode, student.toefl, 90, 70);
    updateDashItemColor(dashCulturalAdaptability.parentNode, student.cultural_adaptability, 70, 40); updateDashItemColor(dashPsychologicalResilience.parentNode, student.psychological_resilience, 70, 40);
    updateDashItemColor(dashLogicSkill.parentNode, student.logic_skill, 70, 40); updateDashItemColor(dashConfidence.parentNode, student.confidence, 70, 40);
    updateDashItemColor(dashAnxiety.parentNode, student.anxiety, 40, 70, true);
}
...
function updateTargetGoalDisplay() {
    ...
    for (const reqKey in gameState.targetGoal.requires) {
        if (gameState.targetGoal.requires.hasOwnProperty(reqKey)) {
            const requiredValue = gameState.targetGoal.requires[reqKey]; const studentValue = gameState.currentStudent[reqKey]; const isMet = studentValue >= requiredValue;
            const reqItem = document.createElement('div'); reqItem.classList.add('goal-requirement-item', isMet ? 'met' : 'unmet');
            let displayKey = reqKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
            if (reqKey === 'gpa') displayKey = 'GPA'; if (reqKey === 'jlpt') displayKey = 'JLPT'; if (reqKey === 'toefl') displayKey = 'TOEFL';
...
async function startGameSimulation() {
    ...
    await sendAiMessage(`欢迎来到【AI 升学破局模拟】！我们将模拟您与一位学生的留学申请全过程。<br><br>您当前辅导的学生是：<strong>${gameState.currentStudent.name}</strong> (${gameState.currentStudent.description})。<br>我们的目标是：<strong>${gameState.targetGoal.name}</strong> (${gameState.targetGoal.description})。<br><br>请点击“抽牌”开始您的第一次策略部署！`);
    updateGameUI(); drawCards(3);
}
...
function updateGameUI() {
    ...
    updateStudentDashboard(); updateTargetGoalDisplay(); renderHand(); renderChallenges();
    if (gameState.selectedCard && gameState.playerEnergy >= (gameState.selectedCard.cost.energy || 0) && gameState.playerInsight >= (gameState.selectedCard.cost.insight || 0) && gameState.playerCredit >= (gameState.selectedCard.cost.credit || 0)) {
        btnApplyStrategy.classList.remove('hidden');
    } else { btnApplyStrategy.classList.add('hidden'); }
}

function renderHand() {
    playerHandContainer.innerHTML = '';
    if (gameState.playerHand.length === 0) { playerHandContainer.innerHTML = '<div class="placeholder-hand">点击“抽牌”获取策略卡</div>'; }
    gameState.playerHand.forEach((card, index) => {
        const cardElement = document.createElement('div'); cardElement.classList.add('card', 'strategy-card');
        if (gameState.selectedCard && gameState.selectedCard.id === card.id && gameState.selectedCard.instanceId === card.instanceId) { cardElement.classList.add('selected'); }
        cardElement.dataset.index = index; cardElement.dataset.cardId = card.id;
        let costDisplay = ''; if (card.cost.energy) costDisplay += `精力: ${card.cost.energy} `; if (card.cost.insight) costDisplay += `洞察: ${card.cost.insight} `; if (card.cost.credit) costDisplay += `信用: ${card.cost.credit} `;
        cardElement.innerHTML = `<div class="card-title">${card.name}</div><div class="card-type">${card.subtype.replace(/_/g, ' ')}</div><div class="card-cost">消耗: ${costDisplay.trim()}</div><div class="card-description">${card.description}</div>`;
...
function renderChallenges() {
    ...
    gameState.activeChallenges.forEach((challenge) => {
        const cardElement = document.createElement('div'); cardElement.classList.add('card', 'challenge-card'); cardElement.dataset.cardId = challenge.id;
        cardElement.innerHTML = `<div class="card-title">${challenge.name}</div><div class="card-type">挑战卡</div><div class="card-description">${challenge.description}<br><span style="color: #ffdddd;">惩罚: ${Object.entries(challenge.penalty).map(([key, value]) => `${key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}: ${value}`).join(', ')}</span>${challenge.solution_strategy_id ? `<br><span style="color: #bbffbb;">推荐策略: ${strategyCards.find(s => s.id === challenge.solution_strategy_id)?.name || '未知'}</span>` : ''}</div>`;
...
async function applyStrategyCard() {
    ...
    for (const solved of solvedChallenges) {
        gameState.activeChallenges = gameState.activeChallenges.filter(c => c.id !== solved.id);
        await sendAiMessage(`挑战 <strong>"${solved.name}"</strong> 已被成功解决！学生属性得到恢复。`);
        for (const attr in solved.penalty) {
            if (gameState.currentStudent[attr] !== undefined) {
                gameState.currentStudent[attr] -= solved.penalty[attr] * 0.5;
                if (attr === 'gpa') gameState.currentStudent[attr] = Math.min(4.0, Math.max(2.0, gameState.currentStudent[attr]));
                else gameState.currentStudent[attr] = Math.min(100, Math.max(0, gameState.currentStudent[attr]));
            }
        }
    }
    gameState.playerHand.splice(gameState.selectedCard.handIndex, 1); gameState.selectedCard = null; btnApplyStrategy.classList.add('hidden');
    updateGameUI();
}

async function parseAndApplyEffect(effectCode) {
    const lines = effectCode.split(';');
    for (const line of lines) {
        const trimmedLine = line.trim(); if (!trimmedLine) continue;
        if (trimmedLine.startsWith('IF ')) {
            const ifPart = trimmedLine.substring(3, trimmedLine.indexOf(' THEN')); const thenPart = trimmedLine.substring(trimmedLine.indexOf(' THEN') + 6);
            let conditionMet = false;
            try {
                if (ifPart.includes("student.has_trait")) {
                    const traitMatch = ifPart.match(/student\.has_trait\(['"]([^'"]+)['"]\)/);
                    if (traitMatch && gameState.currentStudent) conditionMet = gameState.currentStudent.has_trait(traitMatch[1]);
                } else { conditionMet = eval(`(${ifPart})`); }
            } catch (e) { console.error("解析IF条件失败:", ifPart, e); }
            if (conditionMet) await applySingleEffectLine(thenPart);
        } else { await applySingleEffectLine(trimmedLine); }
    }
    updateGameUI();
}

async function applySingleEffectLine(effectStatement) {
    const student = gameState.currentStudent;
    const parts = effectStatement.split(';').map(p => p.trim()).filter(p => p.length > 0);
    for (const part of parts) {
        const attrMatch = part.match(/(student\.\w+)\s*([+\-*/=])\s*(-?\d+)/);
        if (attrMatch) {
            const attrPath = attrMatch[1]; const operator = attrMatch[2]; let value = parseInt(attrMatch[3]);
            if (gameState.selectedCard && gameState.selectedCard.subtype === "psychological_intervention" && gameState.currentPhase === "crisis_phase") {
                value = Math.round(value * (gameState.phaseDescriptions.crisis_phase.psychological_strategy_bonus || 1));
            }
            const attrName = attrPath.split('.')[1];
            if (student && student[attrName] !== undefined) {
                switch (operator) { case '+=': student[attrName] += value; break; case '-=': student[attrName] -= value; break; case '=': student[attrName] = value; break; }
                if (attrName === 'gpa') student[attrName] = Math.min(4.0, Math.max(2.0, student[attrName])); else student[attrName] = Math.min(100, Math.max(0, student[attrName]));
            } continue;
        }
        const gainMatch = part.match(/GAIN\s+(\w+)\s*=\s*(\d+)/);
        if (gainMatch) {
            const resource = gainMatch[1]; const amount = parseInt(gainMatch[2]);
            if (gameState[`player${resource.charAt(0).toUpperCase() + resource.slice(1)}`] !== undefined) {
                gameState[`player${resource.charAt(0).toUpperCase() + resource.slice(1)}`] += amount; await sendAiMessage(`获得 ${amount} ${resource}.`, 200);
            } continue;
        }
        const setStateMatch = part.match(/gameState\.(\w+)\s*=\s*['"]?(\w+)['"]?/);
        if (setStateMatch) { gameState[setStateMatch[1]] = setStateMatch[2]; continue; }
        const alertMatch = part.match(/ALERT\(['"]([^'"]+)['"]\)/);
        if (alertMatch) { await sendAiMessage(alertMatch[1], 500); continue; }
    }
}

async function endTurn() {
    if (gameState.gameOver) return;
    gameState.turn++; await sendAiMessage(`====== 第 ${gameState.turn} 回合结束 ======`);
    updateGamePhase(); await generateNewChallenges();
    gameState.playerEnergy += 5; gameState.playerInsight += 2; drawCards(1);
    await checkWinLoss();
    if (!gameState.gameOver) await sendAiMessage(`进入第 ${gameState.turn} 回合。当前阶段：<strong>${gameState.phaseDescriptions[gameState.currentPhase].name}</strong>。`);
    gameState.selectedCard = null; btnApplyStrategy.classList.add('hidden'); updateGameUI();
}

function updateGamePhase() {
    if (gameState.turn <= 3) gameState.currentPhase = "honeymoon_phase";
    else if (gameState.turn <= 6) { if (gameState.currentPhase !== "crisis_phase") sendAiMessage(`进入【危机期】：文化冲击开始！`, 1000); gameState.currentPhase = "crisis_phase"; }
    else if (gameState.turn <= 9) { if (gameState.currentPhase !== "adjustment_phase") sendAiMessage(`进入【恢复期】：逐步适应。`, 1000); gameState.currentPhase = "adjustment_phase"; }
    else { if (gameState.currentPhase !== "adaptation_phase") sendAiMessage(`进入【适应期】：冲刺目标。`, 1000); gameState.currentPhase = "adaptation_phase"; }
}

async function generateNewChallenges() {
    if (gameState.activeChallenges.length >= 3) return;
    let challengeGenerationChance = 0.2 * (gameState.phaseDescriptions[gameState.currentPhase].challenge_odds_modifier || 1);
    for (let i = 0; i < challengeCards.length; i++) {
        const potentialChallenge = challengeCards[i];
        if (gameState.activeChallenges.some(c => c.id === potentialChallenge.id)) continue;
        let triggerConditionMet = true;
        if (potentialChallenge.trigger) { try { triggerConditionMet = eval(potentialChallenge.trigger); } catch (e) { console.error("挑战触发失败", e); } }
        if (triggerConditionMet && Math.random() < challengeGenerationChance) {
            gameState.activeChallenges.push(potentialChallenge); await sendAiMessage(`新的挑战出现！<strong>"${potentialChallenge.name}"</strong>！`, 700);
            for (const attr in potentialChallenge.penalty) { if (gameState.currentStudent[attr] !== undefined) gameState.currentStudent[attr] += potentialChallenge.penalty[attr]; }
            break;
        }
    }
}
...
async function checkWinLoss() {
    const student = gameState.currentStudent; const goal = gameState.targetGoal;
    let allRequirementsMet = true;
    for (const reqKey in goal.requires) { if (student[reqKey] < goal.requires[reqKey]) { allRequirementsMet = false; break; } }
    if (allRequirementsMet) { gameState.gameOver = true; showGameResult("挑战成功！", "var(--status-positive)", goal.pass_message); return; }
    if (gameState.turn >= gameState.maxTurns) { gameState.gameOver = true; showGameResult("时间耗尽！", "var(--status-negative)", goal.fail_message); return; }
    if (student.anxiety >= 90) { gameState.gameOver = true; showGameResult("学生情绪崩溃！", "var(--status-negative)", `焦虑过高，申请失败！${goal.fail_message}`); return; }
    if (student.confidence <= 10) { gameState.gameOver = true; showGameResult("学生信心全无！", "var(--status-negative)", `自信心跌至谷底，申请失败！${goal.fail_message}`); return; }
}

function showGameResult(title, color, message) {
    gameResultScreen.classList.remove('hidden');
    gameResultScreen.querySelector('h3').textContent = title;
    gameResultScreen.querySelector('h3').style.color = color;
    gameResultText.innerHTML = message;
}

function backToChat() {
    gameSimulationSection.classList.add('hidden'); chatSection.classList.remove('hidden'); profileCover.classList.remove('hidden');
    gameState.gameStarted = false; gameState.gameOver = false;
    if (menuList) menuList.classList.add('hidden');
    if (contentDetail) contentDetail.classList.add('hidden');
    chatBody.scrollTop = chatBody.scrollHeight;
    sendAiMessage("欢迎回到咨询室，有什么新问题吗？");
}

// --- 6. 事件监听器与初始化 ---
document.addEventListener('DOMContentLoaded', () => {
    // 修复：检查关键 DOM 元素是否存在
    if (!userInput || !sendButton || !btnDrawCards || !btnApplyStrategy || !btnEndTurn || !btnBackToChat || !btnRestartGame) {
        console.error("一个或多个 UI 按钮/输入框未找到，请检查 HTML IDs。");
        return;
    }

    userInput.addEventListener('keypress', (e) => { if (e.key === 'Enter') sendMessage(); });
    sendButton.addEventListener('click', sendMessage);
    btnDrawCards.addEventListener('click', () => drawCards(1));
    btnApplyStrategy.addEventListener('click', applyStrategyCard);
    btnEndTurn.addEventListener('click', endTurn);
    btnBackToChat.addEventListener('click', backToChat);
    btnRestartGame.addEventListener('click', startGameSimulation);
    document.getElementById('btnExpand').addEventListener('click', () => { profileCover.classList.add('hidden'); menuList.classList.remove('hidden'); });
    document.getElementById('btnBackHome').addEventListener('click', () => { menuList.classList.add('hidden'); profileCover.classList.remove('hidden'); });
    document.getElementById('btnBackMenu').addEventListener('click', () => { contentDetail.classList.add('hidden'); menuList.classList.remove('hidden'); });

    document.querySelectorAll('.menu-item').forEach(item => {
        item.addEventListener('click', async function() {
            const targetId = this.dataset.target;
            if (targetId) {
                menuList.classList.add('hidden'); contentDetail.classList.remove('hidden');
                dynamicContent.innerHTML = '';
                if (targetId === 'snsReview') {
                    dynamicContent.innerHTML = `<div class="detail-card"><h3>SNS评论生成器</h3><textarea id="snsPostContent" rows="8" placeholder="输入帖子内容..."></textarea><button id="generateSnsComment" class="link-btn" style="margin-top: 20px;">生成评论</button><div id="snsCommentResult" style="margin-top: 20px; white-space: pre-wrap; background-color: #f9f9f9; padding: 15px;"></div></div>`;
                    document.getElementById('generateSnsComment').addEventListener('click', () => {
                        document.getElementById('snsCommentResult').textContent = SNS_COMMENT_GENERATOR.generate(document.getElementById('snsPostContent').value);
                    });
                } else {
                    const keyword = this.querySelector('h3').textContent.split(' ')[0];
                    dynamicContent.innerHTML = `<div class="detail-card"><h3>${this.querySelector('h3').textContent}</h3><p>${qaDatabase[keyword] || '暂无详细信息。'}</p></div>`;
                }
            }
        });
    });
    ...
    sendAiMessage("您好！我是您的AI留学咨询师。有什么问题，或者想体验【AI升学破局模拟】吗？");
    updateGameUI();
});

async function sendMessage() {
    const message = userInput.value.trim(); if (message === '') return;
    sendUserMessage(message); userInput.value = '';
    let aiResponse = getRandomElement(strategicFallbackResponses);
    for (const keyword in qaDatabase) { if (message.includes(keyword)) { aiResponse = qaDatabase[keyword]; break; } }
