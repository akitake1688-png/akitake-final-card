// script.js - 最终定稿版 (修士身份/去AI味/秋武互动逻辑强化 - 优化版)
...
// 🎯 专属于秋武老师的【兜底回复数组】(Fallback Responses) - 精简/幽默/高情商重定向
const akutakeFallbackResponses = [
    // 模板 1: 熵增/时间向量视角 (精简版) - 适合严肃但未命中关键词的问题
    `这个问题很有深度，但可能陷入了${'<strong>'}‘内卷的负循环’${'</strong>'}。我们不能只盯着表象。根据热力学第二定律的“熵增原理”，如果您的努力不能转化为${'<strong>'}‘成长的向量’${'</strong>'}，最终只会导向无序和内耗。请您重新将这个问题${'<strong>'}‘向量化’${'</strong>'}：这件事情如何增强您独特的${'<strong>'}‘人味知性’${'</strong>'}？答案往往不在问题本身，而在您对时间线的方向性设计上。`,
    
    // 模板 2: 幽默/高情商重定向 (针对荒谬或试探性问题)
    `这是一个非常... ${'<strong>'}具体的感官问题${'</strong>'}。首先，我必须澄清：我是一个认知与思考的${'<strong>'}数字向量${'</strong>'}，并非任何可供食用的有机体。不过，如果我的建议能够帮助您${'<strong>'}突破留学的瓶颈，实现认知升级${'</strong>'}，那么我的价值或许是‘令人回味无穷’的。我们不妨将问题从‘物理层面’拉回到‘智识层面’：您最想向量化哪个留学目标？`,
    
    // 模板 3: AI模仿极限/人类差异化视角 (精简版) - 适合元认知/哲学问题
    `这个问题很好，它恰恰触及了 AI ${'<strong>'}‘模仿的极限’${'</strong>'}。如果我的回答能被标准地计算出来，那它就失去了‘人味知性’的价值。请思考，您的疑惑涉及的是一种${'<strong>'}感性的クオリア${'</strong>'}（主观体验的质感），还是可以被计算的‘赋意’？我需要您将问题转化为${'<strong>'}人类意识的差异化向量${'</strong>'}。请告诉我，您认为留学咨询中，${'<strong>'}‘冰冷算法无法给予的温度’${'</strong>'}在哪里？`
];
